...
const main = async () => {
    ...
    console.log("Terminating Loan (from Borrower)");
    try {
        const tx = await loanRequestContract.terminateLoan({ value: ethers.utils.parseEther("1.1") });
        await tx.wait();
        console.log("Compensated with NFT");
    }
    catch (e) {
        console.log(e);
    }
